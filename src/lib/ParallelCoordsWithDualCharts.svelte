<script>
  ...
  onMount(async () => {
    ...
    const metrics = ["PRECIPITATION", "MAX_TEMP", "MIN_TEMP", "AVG_WIND_SPEED", "TEMP_RANGE", "WIND_TEMP_RATIO"];
  ...
  function updateVisualization() {
    ...
    Plotly.react(chartDiv, [{
      ...
      line: {
        ...
        colorscale: "Jet",
        showscale: true,
        colorbar: { title: colorMetric.replace(/_/g, " ") }
      },
    ...
    dimensions.forEach((dim, i) => {
      if (dim.constraintrange) {
        ...
        filtered = filtered.filter(d =>
          constraintRanges.some(([min, max]) => d[metric] >= min && d[metric] <= max)
        );
      }
    });
